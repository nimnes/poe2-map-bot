...
class BotState:
    def __init__(self, data_path: Path):
        ...
        self.map_book = MapBook.load(data_path)

    def reload(self) -> None:
        self.map_book = MapBook.load(self.data_path)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.effective_message.reply_text(
        ...
        f"I currently know {len(state.map_book.profiles)} maps."
    )
...
async def reload_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    state = get_state(context)
    state.reload()
    await update.effective_message.reply_text(f"Reloaded {len(state.map_book.profiles)} map profiles.")
...
def build_application() -> Application:
    ...
    data_path = Path(os.getenv("MAP_DATA_PATH", "data/maps.json")).resolve()
    state = BotState(data_path)
